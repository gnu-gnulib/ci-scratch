  ...
  if (supersede_if_exists)
    {
      if (supersede_if_does_not_exist)
        ...
        {
fprintf (stderr, "open_supersede(true,false)1: filename=%s, flags=0x%x, mode=0%o\n", filename, flags, mode);
          fd = open (filename, flags | O_CREAT | O_EXCL, mode);
fprintf (stderr, "open_supersede(true,false)1: fd = %d, errno = %d\n", fd, errno);
          if (fd >= 0)
            {
          ...
          else
            {
              ...
              char *canon_filename =
                canonicalize_filename_mode (filename, CAN_ALL_BUT_LAST);
fprintf (stderr, "open_supersede(true,false): canon_filename = %s\n", canon_filename != NULL ? canon_filename : "(null)");
              if (canon_filename == NULL)
                fd = -1;
              else
                {
fprintf (stderr, "open_supersede(true,false)2: canon_filename=%s, flags=0x%x, mode=0%o\n", canon_filename, flags, mode);
                  fd = open (canon_filename, flags | O_CREAT | O_EXCL, mode);
fprintf (stderr, "open_supersede(true,false)2: fd = %d, errno = %d\n", fd, errno);
                  if (fd >= 0)
                    {
                      ...
                      if (stat (canon_filename, &statbuf) >= 0
                      ...
                      else
                        {
                          /* It is possibly a character device, socket, or
                             something like that.  */
fprintf (stderr, "open_supersede(true,false)3: canon_filename=%s, flags|extra_flags=0x%x, mode=0%o\n", canon_filename, flags | extra_flags, mode);
                          fd = open (canon_filename, flags | extra_flags, mode);
fprintf (stderr, "open_supersede(true,false)3: fd = %d, errno = %d\n", fd, errno);
                          free (canon_filename);
                          if (fd >= 0)
                            {
                              action->final_rename_temp = NULL;
